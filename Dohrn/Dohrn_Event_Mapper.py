...
class post:
    ...
    def EventPost(self, pronumber, LTLpath, path, config_default, logger, kafka_publisher):
        ...
        dohrndfservice = dohrndfservice.rename(
            columns={"Total:": "Pieces", "Pieces:": "Pallets", "Pallets:": "Weight", "Weight:": "Service Type"})
        ...
        for i in range(0, len(dohrndf)):
            ...
            try:
                ...
                postJson["pickupDate"] = pd.datetime.datetime.strptime(dohrndfstatus.loc["Ship Date:"]['Data'],
                                                                       "%m/%d/%Y").strftime(
                    "%Y-%m-%d")  # here ship date
                #   print(postJson["pickupDate"])
                postJson["deliveryDate"] = pd.datetime.datetime.strptime(dohrndfstatus.loc['Delivered Date:']['Data'],
                ...
                postJson["eventCode"], postJson["eventName"] = self.Event(eventMap, eventSet,
                                                                          dohrndf.iloc[i]['Shipment Status'], logger)
                #  print(postJson["eventCode"])
                logger.info(postJson["eventCode"])
                logger.info(postJson["eventName"])

                #   print(postJson["eventName"])
                postJson["eventDate"] = datetime.datetime.strptime(dohrndf.loc[i]['Date'], "%m/%d/%Y").strftime(
                    "%m-%d-%Y")
                postJson["appointmentDate"]=""
                postJson["appointmentTimeStart"]=""
                postJson["appointmentTimeEnd"]=""
